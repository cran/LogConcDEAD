'lcd.weights' <- function(X, weights=NULL, initial=NULL, verbose=-1, alpha=5, c=1, sigmatol=10^-8, integraltol=10^-4, ytol=10^-4, stepscale=5.1, stepscale2=2,stepscale3=1.5,stepscale4=1.05,desiredsize=3.3,Jtol=0.001) {
##  if(!require("geometry",quietly=TRUE)) stop("you need to install the geometry package")
  ...
  out <- .C("logconestw",yvalue=as.double(y),as.double(X),as.integer(ncol(X)),as.integer(nrow(X)),as.double(weights),options= as.double(opts),minvalue=double(1),parameters=as.double(parameters),Jtol=as.double(Jtol),nouter=as.integer(nouter),PACKAGE="LogConcDEAD")
  ...
  r <- list(x=X,w=weights,logMLE=out$yvalue,NumberOfEvaluations=out$options[9:11],MinSigma=out$minvalue,b=b,beta=beta,chull=ConvHull,verts=verts,vertsoffset=vertsoffset)
  class(r) <- "LCDweights"
  return(r)}
